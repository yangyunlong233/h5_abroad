<template>
  ...
  <button @click="tstGet">发起请求</button>
  <button @click="vvv">测试请求</button>
  <div>{{ showTst }}</div>
...
export default {
  ...
  setup () {
    ...
    const showTst = ref('测试内容')
    // 测试
    const tstGet = () => {
      accessToken.value = localStorage.getItem('access_token')
      console.log('获取的token', accessToken.value)
      $axios.get('/api/token/info', {
        ...
        .then(response => {
          console.log('测试结果1', response)
          showTst.value = response.data.data
        })
    }

    const vvv = () => {
      $axios.post('/api/user/auth/notify',
        {
          behavior: 5,
          key: captchaVerification,
          mobile: mobile.value,
        ...
        .then(response => {
          console.log('测试结果2', response.data)
        })
    }
